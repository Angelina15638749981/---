...
base_dir = "./"
path_test_image = os.path.join(base_dir, "test.jpg")
...
cv2.imwrite("binaryimage.png",adaptive_threshold)
...
cv2.imwrite("lineimage.png", line_seg_adaptive_threshold)
...
text = pytesseract.image_to_string(image_color, lang='chi_sim')  # 读取文字，此为默认英文
print("出票人全称：%s" % text.replace("  ", "").replace("”", "").replace("“", "").replace("。", ""))